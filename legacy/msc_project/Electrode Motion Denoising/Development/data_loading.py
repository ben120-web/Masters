...
def load_data(clean_signals_path, noisy_signals_path, segment_length=1500, batch_size=1, num_workers=2):
    ...
    noisy_signals = {snr: {} for snr in ['SNR0', 'SNR6', 'SNR12', 'SNR18', 'SNR24']} #'SNR0', 'SNR6', 'SNR12', 'SNR18', 
...
def load_data_for_inference(file_path):
    """Loads a user-uploaded HDF5 file for inference."""
    if not os.path.exists(file_path):
        print(f"File {file_path} does not exist.")
        return None
    
    ecg_signal = load_h5_file(file_path)
    return ecg_signal
